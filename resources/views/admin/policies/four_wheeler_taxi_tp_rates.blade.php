...
<div class="row ">
    <h3 class="text-center"> Four Wheeler Up to 6 Passangers Taxi Tp Charges </h3>
    <div class="col-lg-12 margin-tb mx-auto ">
      ...
      <a href="{{route('admin.four_wheeler_upto_6_passengers_taxi.index')}}" class="btn btn-info">Go Back</a>
...
<table id="dtHorizontalExample" class="table table-striped  table-bordered table-dark ">
              ...
              <thead>
                <tr>
                  ...
                  <th>CC</th>
                  <th>TP Rate</th>
                  <th>Tp Rate Per Passanger</th>
                  <th>Action</th>
                </tr>
              </thead>
         
          @if ( isset($four_wheeler_taxi_cc_tp_charges))
       
               <tbody>

                @foreach ($four_wheeler_taxi_cc_tp_charges as $key =>  $data)

                  <tr>
                    <td>{{++$key}}</td>
                    <td>{{$data->cc}}</td>                       
                    <td>{{$data->tp_rate}}</td>
                    <td>{{$data->rate_per_passanger}}</td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
                ...
                <div class="container" >
                  ...
                  <form action="{{route('admin.four_wheeler_upto_6_passengers_taxi.tp.store')}}"  method="POST" id="taxi_cc_tp_create" enctype="multipart/form-data" >

                          @csrf                

                    <div class="form-group">
                      <label for="">Vehicle CC</label>
                      <input type="text" class="form-control" value="" min="0" name="cc" required>
                      <span class="text-danger error-text cc_error "></span>
                    </div>

                    <div class="form-group">
                      <label for="">TP Rate </label>
                      <input type="text" class="form-control" value="" min="0" name="tp_rate" required>
                      <span class="text-danger error-text tp_rate_error "></span>
                    </div>

                    <div class="form-group">
                      <label for="">Rate Per Passangers</label>
                      <input type="text" class="form-control" value="" min="0" name="rate_per_passanger" required>
                      <span class="text-danger error-text rate_per_passanger_error "></span>
                    </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
                ...
                <div class="container" >
                <form action="{{route('admin.four_wheeler_upto_6_passengers_taxi.tp.store')}}"  method="POST" id="two_wheeler_update" enctype="multipart/form-data" >
                      @csrf                  
                    ...
                    <div class="form-group">
                      <label for="">Rate Per Passangers</label>
                      <input type="text" class="form-control" value="" min="0" name="rate_per_passanger" id="edit_rate_per_passanger" required>
                      <span class="text-danger error-text rate_per_passanger_error "></span>
                    </div>
...
<script>
    ...
    $(document).ready( function(){
      ...
      $("#taxi_cc_tp_create").on('submit',function(e){
  ...
  $(document).on('click','.edit_policy',function(e){
      ...
      type:"GET",
      url:  "{{APP_PATH}}"+"admin/four-wheeler-upto-6-passengers-taxi/tp/edit/"+rate_chart_id,
            ...
            if(response.status == 404){
                ...
                $('#edit_cc').val(response.taxi_cc_tp_data.cc);
                $('#edit_tp_rate').val(response.taxi_cc_tp_data.tp_rate );
                $('#edit_rate_per_passanger').val(response.taxi_cc_tp_data.rate_per_passanger );
...
$.ajax({
    ...
    type:"DELETE",
    url: "{{APP_PATH}}"+"admin/four-wheeler-upto-6-passengers-taxi/tp/delete/"+policy_id,
