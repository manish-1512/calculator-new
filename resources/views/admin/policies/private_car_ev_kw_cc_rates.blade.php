...
<div class="row ">
    <h3 class="text-center">Pravate Car EV  KW  AND TP Rates  </h3>
...
<table id="dtHorizontalExample" class="table table-striped  table-bordered table-dark ">
          ...
          @if ( isset($private_car_ev_kw_tp_charges))
       
               <tbody>

                @foreach ($private_car_ev_kw_tp_charges as $key =>  $data)

                  <tr>
                    <td>{{++$key}}</td>
                    <td>{{$data->kw}}</td>                       
                    <td>{{$data->tp_one_year}}</td>
                    <td>{{$data->tp_three_year}}</td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
                ...
                <div class="container" >
                  ...
                  <form action="{{route('admin.private_car_ev.kw_tp.store')}}"  method="POST" id="private_car_ev_kw_tp_create" enctype="multipart/form-data" >

                          @csrf                

                    <div class="form-group">
                      <label for="image_order">Vehicle KW </label>
                      <input type="text" class="form-control" value="" min="0" name="kw"  required>
                      <span class="text-danger error-text kw_error "></span>
                    </div>
                    ...
                    <div class="form-group">
                      <label for="image_order">TP Three Year</label>
                      <input type="text" class="form-control" value="" min="0" name="tp_three_year"  required>
                      <span class="text-danger error-text tp_three_year_error "></span>
                    </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
                ...
                <div class="container" >
                <form action="{{route('admin.private_car_ev.kw_tp.store')}}"  method="POST" id="two_wheeler_update" enctype="multipart/form-data" >
                      @csrf                  
                      ...
                      <input type="text" class="form-control" value="" min="0" name="kw" id="edit_kw" required>
                      <span class="text-danger error-text kw_error "></span>
                    </div>
                    ...
                    <div class="form-group">
                      <label for="image_order">TP Three Year</label>
                      <input type="text" class="form-control" value="" min="0" name="tp_three_year" id="edit_tp_three_year" required>
                      <span class="text-danger error-text tp_three_year_error "></span>
                    </div>
...
<script>
    ...
    $(document).ready( function(){
      ...
      $("#private_car_ev_kw_tp_create").on('submit',function(e){
                 ...
                 success:function(data){
                  ...
                  console.log(data);
                      ...
                      if(data.status == 401){
                        ...
                        $('#private_car_ev_kw_tp_create')[0].reset();
  ...
  $(document).on('click','.edit_policy',function(e){
      ...
      $.ajax({
      type:"GET",
      url:  "{{APP_PATH}}"+"admin/private-car-ev/kw-tp/edit/"+rate_chart_id,
            ...
            if(response.status == 404){
                ...
                $('#edit_kw').val(response.private_car_ev_kw_tp_data.kw);
                $('#edit_tp_one_year').val(response.private_car_ev_kw_tp_data.tp_one_year );
                $('#edit_tp_three_year').val(response.private_car_ev_kw_tp_data.tp_three_year );
...
$.ajax({
    ...
    type:"DELETE",
    url: "{{APP_PATH}}"+"admin/private-car-ev/kw-tp/delete/"+policy_id,
