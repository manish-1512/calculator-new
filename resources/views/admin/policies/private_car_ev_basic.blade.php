...
<div class="row ">
    <h3 class="text-center">Basic rates for Private Car EV  </h3>
...
@if ( isset($private_car_ev_rate_chart) )

     <tbody>


      @foreach ($private_car_ev_rate_chart as $key =>  $data)
...
<div class="modal-body">
      ...
      <div class="container" >
        ...
        <form action="{{route('admin.private_car_ev.store')}}"  method="POST" id="private_car_ev_create" enctype="multipart/form-data" >
          ...
          <div class="form-group">
            ...
            <select name="kw" id="" class="form-control" required> 

            <option >Select Kilowatt</option>

               @if(isset($kw_and_tp_for_private_car)) 

                    @foreach ($kw_and_tp_for_private_car as $kw)
...
<div class="modal-body">
      ...
      <div class="container" >
      <form action="{{route('admin.private_car_ev.store')}}"  method="POST" id="private_car_update" enctype="multipart/form-data" >
            @csrf                  
          ...
          <div class="form-group">
            <label for="kw">Vehicle  Engine  </label>

            <select name="kw" id="edit_kw" class="form-control" required> 
...
$("#private_car_ev_create").on('submit',function(e){
...
type:"GET",
url:  "{{APP_PATH}}"+"admin/private-car-ev/edit/"+rate_chart_id,
  ...
  if(response.status == 404){
      ...
      $('#edit_zone').val(response.private_car_ev_data.zone );
      $('#edit_age').val(response.private_car_ev_data.age);
      $('#edit_kw').val(response.private_car_ev_data.kw);
      $('#edit_tp_one_year').val(response.private_car_ev_data.tp_one_year );
      $('#edit_vehicle_basic_rate').val(response.private_car_ev_data.vehicle_basic_rate );
...
$("#private_car_update").on('submit',function(e){
...
var policy_id = $('#delete_policy_id').val();
    ...
    type:"DELETE",
    url: "{{APP_PATH}}"+"admin/private-car-ev/delete/"+policy_id,
