...
class GoodsCarryingPublicController extends Controller
{
    public function calcuatePolicyPremiun(Request $request){
        ...
        $validator = Validator::make($request->all(), [   
            ...
            'mit_23' => "required",
            'non_electrical_accessories' => "required", 
        ...
        if($validator->fails()){
            ...
            return response()->json(['status' => 401 ,'error' => $validator->errors()->toArray() ]);
                    ...
                    $non_electrical_accessories =  $request->non_electrical_accessories;

                    $lpg_cng_kit = $request->lpg_cng_kit; 

                    $external_lpg_cng_kit_price = $request->external_lpg_cng_kit_price;                   
                    ...
                    $chart =  GoodsCarryingVehicle_public_other_then_three_wheeler::where('zone',$zone)->where('age',$age_of_vehicle)->first();
                   
                    //od premium
                    //to be continue
                         $cc_tp_charges =  GoodsCarryingVehicle_public_other_then_three_wheeler_tp_rates::where('id',$cubic_capacity)->first();           
                                ...
                                $non_electrical_accessories = (($non_electrical_accessories * 3.283) / 100 ) ;

                                $lpg_cng_kit = ($lpg_cng_kit == 1)? 60 : 0 ; 
                                $external_lpg_cng_kit_price = ($external_lpg_cng_kit_price * 4 ) / 100 ; 
                                
                                $total_basic_premium = $basic_od_premium_after_discount + $electrical_accessories + $non_electrical_accessories + $lpg_cng_kit + $external_lpg_cng_kit_price;
                    ...
                    $own_damage_premium=  [
                                ...
                                "vehicle_basic_rate" => $chart->vehicle_basic_rate,
                                "tp_one_year" => $cc_tp_charges->tp_one_year,
                                "tp_five_year" => $cc_tp_charges->tp_three_year,
                                "as" => $cc_tp_charges
                    ...
                    $liablity_premium = [
                        ...
                        "total_b" => ($cc_tp_charges->tp_one_year + $pa_to_owner_driver + $ll_to_paid_driver+ $pa_to_unnamed_passenger + $lpg_cng_kit - $restriccted_tppd )
